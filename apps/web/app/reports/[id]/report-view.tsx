...
const WORKER_STEPS: WorkerStep[] = [
  {
    key: "w1",
    label: "Analyzing market demand",
    sub: "Import data, pricing, and tariffs",
  },
  {
    key: "w2",
    label: "Checking compliance",
    sub: "EU requirements for your product",
  },
  {
    key: "w3",
    label: "Discovering buyer companies",
    sub: "Searching Apollo and trade directories",
  },
  {
    key: "w4",
    label: "Analyzing sourcing signals",
    sub: "Job postings, fairs, and growth data",
  },
  {
    key: "w5",
    label: "Writing outreach emails",
    sub: "Personalising for each buyer",
  },
];
...
const FLAG_STYLES: Record<string, React.CSSProperties> = {
  DE: {
    background:
      "linear-gradient(180deg,#000 0 33%,#DD0000 33% 66%,#FFCC00 66%)",
  },
  AT: {
    background:
      "linear-gradient(180deg,#ED2939 0 33%,#fff 33% 66%,#ED2939 66%)",
  },
  IT: {
  ...
  NL: {
    background:
      "linear-gradient(180deg,#AE1C28 0 33%,#fff 33% 66%,#21468B 66%)",
  },
  CH: { background: "#DA291C" },
  BE: {
    background: "linear-gradient(90deg,#000 0 33%,#FAE042 33% 66%,#ED2939 66%)",
  },
  ...
  DK: {
    background:
      "linear-gradient(180deg,#C8102E 0 33%,#fff 33% 55%,#C8102E 55%)",
  },
  GB: {
    background:
      "linear-gradient(45deg,#012169 0 40%,#fff 40% 50%,#C8102E 50% 60%,#fff 60% 70%,#012169 70%)",
  },
};
...
const PIPELINE_BUCKETS: {
...
}[] = [
  {
    ...
    label: "Not contacted",
    dot: "#9CA3AF",
    chip: "bg-muted text-ink-3",
  },
  {
    key: "contacted",
    label: "Contacted",
    dot: "#1F3B5C",
    chip: "bg-[#EAF1FA] text-[#1F3B5C]",
  },
  {
    key: "replied",
    label: "Replied",
    dot: "#5A4416",
    chip: "bg-[#FBF1DA] text-[#5A4416]",
  },
  {
    key: "meeting",
    label: "Meeting booked",
    dot: "#9CC129",
    chip: "bg-[#EBF6C8] text-green-ink",
  },
  {
    key: "deal",
    label: "Deal closed",
    dot: "#5C1F38",
    chip: "bg-[#FAEAF0] text-[#5C1F38]",
  },
];
...
export function ReportView({ report: initialReport }: { report: any }) {
  ...
  const [completedWorkers, setCompletedWorkers] = useState<number>(
    initialReport.status === "complete"
      ? 5
      : (initialReport.current_worker ?? 0),
  );
  ...
  useEffect(() => {
    ...
    fetch(`/api/reports/${report.id}/run`, { method: "POST" })
      .then(async (res) => {
        if (!res.ok) {
          ...
          const msg =
            typeof json.detail === "string"
              ...
              : typeof json.error === "string"
                ? json.error
                : `Failed to start pipeline (${res.status})`;
          toast.error(msg);
        }
      })
      .catch(() =>
        toast.error(
          "Network error — could not start the pipeline. Please refresh.",
        ),
      );
  }, [report.id, status]);
...
function ProcessingView({
  ...
  const pct = Math.min(
    99,
    Math.max(8, Math.round((completedWorkers / WORKER_STEPS.length) * 100)),
  );
    ...
    <div className="flex min-h-[calc(100vh-3.5rem)] items-center justify-center px-6 py-10 bg-white overflow-auto">
      <div className="relative w-full max-w-[560px] bg-warm-1 rounded-[28px] px-11 pt-11 pb-9 shadow-[0_1px_0_rgba(80,60,30,0.04),0_30px_60px_-30px_rgba(99,73,33,0.30)] overflow-hidden">
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(rgba(99,73,33,0.06) 1px, transparent 1px)",
          ...
          {userEmail && (
            <p className="mt-3.5 text-center text-xs text-[#7a6741]">
              ...
              <a
                ...
                className="text-warm-ink underline underline-offset-2"
              >
                {userEmail}
              </a>{" "}
  ...
  const buyers: any[] = useMemo(
    () =>
      [...(report.report_buyers ?? [])]
        ...
        .sort(
          (a, b) => (b.receptiveness_score ?? 0) - (a.receptiveness_score ?? 0),
        ),
    [report.report_buyers],
  );
    ...
    <div className="flex flex-1 min-h-0 h-[calc(100vh-3.5rem)]">
      ...
      <aside className="w-[280px] shrink-0 bg-[#F9FAFB] border-r border-line flex flex-col min-h-0 max-[720px]:hidden">
        <div className="px-5 pt-4 pb-2 shrink-0">
          ...
          <h2 className="font-serif text-[22px] tracking-tight leading-tight text-ink">
            {listTitle}
          </h2>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-1.5 mt-1 text-[11.5px] text-ink-3">
                  <CountryFlag
                    iso2={buyer.country_iso2 ?? report.target_iso2}
                    size={14}
                  />
                  {buyer.city ??
                    COUNTRY_NAMES[buyer.country_iso2] ??
                    buyer.country_iso2}
                </div>
                <span
                  className={cn(
                    "inline-flex items-center gap-1 mt-1.5 px-1.5 py-0.5 rounded-md text-[10.5px] font-medium",
                    bucket.chip,
                  )}
                >
                  <span
      ...
      <section className="flex-1 min-w-0 overflow-y-auto bg-white px-9 py-7 pb-16 max-[1180px]:px-7">
        {selectedBuyer ? (
          <LeadDetail
            ...
            onMarkContacted={() =>
              updatePipeline(selectedBuyer.id, "contacted")
            }
          />
        ) : (
          <div className="h-full flex flex-col items-center justify-center text-center text-ink-3">
            <TrendingUp
              className="w-14 h-14 text-ink-5 mb-4 opacity-40"
              strokeWidth={1.4}
            />
            <h3 className="font-serif text-2xl text-ink tracking-tight mb-2">
              Select a lead to view their profile
            </h3>
            <p className="text-sm">
              Click any lead in the sidebar to get started
            </p>
          </div>
        )}
      </section>
      ...
      <aside className="w-[340px] shrink-0 border-l border-line bg-white flex flex-col min-h-0 max-[900px]:hidden">
        ...
        <div className="px-5 py-4 border-t border-line text-xs text-ink-3 shrink-0">
          <span className="font-semibold text-ink">{activeConversations}</span>{" "}
          active conversation
          {activeConversations !== 1 ? "s" : ""} ·{" "}
          <span className="font-semibold text-ink">{dealsClosed}</span> deals
          closed
        </div>
      </aside>
    </div>
...
function LeadDetail({
  ...
  console.log(buyer, report);
  const [copied, setCopied] = useState(false);
  const [logoUrl, setLogoUrl] = useState<string | null>(
    buyer.company_domain
      ? `https://logo.clearbit.com/${buyer.company_domain}`
      : null,
  );
  ...
  const descriptionSignal = signals.find((s) =>
    s.startsWith("Company description:"),
  );
  const companyDescription = descriptionSignal
    ? descriptionSignal.replace("Company description:", "").trim()
    : `Leading buyer of ${report.hs_code ? `HS ${report.hs_code}` : "wholesale"} products, active in the target region.`;
  ...
  return (
    <>
        ...
        <div className="flex items-start gap-4">
          {logoUrl ? (
            <img
              ...
              className="w-14 h-14 rounded-xl border border-line bg-white object-contain p-1 shrink-0 mt-1"
              onError={() => setLogoUrl(null)}
            />
          ) : (
            <div className="w-14 h-14 rounded-xl border border-line bg-[#F9FAFB] flex items-center justify-center shrink-0 mt-1 text-ink-3 font-semibold text-lg">
              {getInitials(buyer.company_name)}
            </div>
          )}
          <div>
            <h1 className="font-serif text-3xl font-bold tracking-tight leading-tight text-ink">
              {buyer.company_name}
            </h1>
            ...
            {buyer.company_domain && (
              <div className="mt-2">
                <a
                  href={
                    buyer.company_domain.startsWith("http")
                      ? buyer.company_domain
                      : `https://${buyer.company_domain}`
                  }
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-[13px] text-green-deep hover:underline font-semibold"
                >
      ...
      <div className="py-5 border-b border-line">
        <p className="text-[11px] font-mono-brand uppercase tracking-[0.08em] text-ink-3 mb-2">
          Company Overview
        </p>
      ...
      {signalCards.length > 0 && (
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-3.5">
          {signalCards.map((card) => (
            <div
              key={card.title}
              className="border border-line rounded-[14px] p-4"
            >
      ...
      {(buyer.contact_name || buyer.contact_email) && (
        <div className="mt-5 border border-line rounded-[14px] px-5 py-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
              ...
              <p className="text-base font-semibold text-ink">
                {buyer.linkedin_url ? (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:text-green-deep hover:underline inline-flex items-center gap-1"
                  >
                    {buyer.contact_name}
                    <ExternalLink className="w-3.5 h-3.5 inline text-ink-4" />
                  </a>
                ) : (
                  buyer.contact_name
                )}
            ...
            {whyPriority && (
              <p className="mt-2 text-xs text-ink-3 italic">{whyPriority}</p>
            )}
          </div>
...
function CountryFlag({ iso2, size = 20 }: { iso2: string; size?: number }) {
  const style = FLAG_STYLES[iso2] ?? {
    background: "linear-gradient(135deg,#cfcfcf,#9ca3af)",
  };
  return (
    <span
...
function scoreBreakdown(signals: string[]) {
  ...
  for (const signal of signals) {
    if (signal.startsWith("Import diversification+:")) structural += 10;
      ...
      growth += signal.includes("stable") ? 8 : 15;
  ...
  return {
    ...
    reach: Math.min(100, Math.round((reach / 20) * 100)),
  };
}

function buildSignalCards(
  signals: string[],
  buyer: { contact_email?: string; linkedin_url?: string },
) {
  const cards: {
    title: string;
  ...
  const reachSig = signals.find(
    (s) => s.startsWith("Decision-maker") || s.startsWith("Trade fair"),
  );
